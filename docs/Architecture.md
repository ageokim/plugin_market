...
- **디자인 언어 — "카페" 테마** (2026-07 사용자 제공 시안으로 전면 교체): 크림 배경(`#f7f1e8` + 앰버/세이지 라디얼 글로우) 위 **테라코타→살몬 그라데이션**(`135deg #c96a4e→#e59578`) 액센트, 서피스 `#fffdf8`·에스프레소 텍스트 `#3d2f24`. 타이포는 **Gowun Batang(세리프 — 브랜드·섹션 라벨·preset/org명) + Noto Sans KR(본문)**, 폰트는 `web/vendor/fonts/` 동봉(CDN 금지 §13.2). 터미널은 로스티드 브라운(`#3a2e26`). 아이콘은 **인라인 SVG 스트로크 스프라이트**(`<symbol>`/`<use>`) — 커피잔+스팀(브랜드), 원두(플러그인), 메뉴판 스윙(preset), 드리퍼(RECIPE), 포터필터+커피가루(org 추가), 에스프레소 추출(전송), 문(입장) 커스텀 아이콘이며, 애니메이션은 use shadow 안에서도 도는 **SMIL**로 스프라이트에 내장. 상태 표시는 색점(●)+라벨. 기준 시안: `docs/mockup/Plugin Cafe - *.html`(로컬 참조, git 제외)·실구현 스냅샷 docs/mockup/.
...
- 상태 표시명(UI 한국어 — 카페 카피): `Enabled` → **추출중**(초록점), `Installed` → **보관중**(골드점), `Available` → **재료 없음**(점 없음·탠) — 내부 상태명(§6.4)과 CLI 출력은 영문 유지. 액션 카피: install 버튼은 **[담기]**, 로그인 버튼은 **[입장하기]**(§12.6).
- 카페 렌더 구조: org 그룹은 **레시피 카드**(색 헤더 — 앰버/세이지 2톤 순환 + 드리퍼 아이콘 + `재료 N가지 · M가지 준비됨` + RECIPE 필), preset 섹션은 **TODAY'S PRESET 메뉴판**(dashed 보드 + dotted leader), 플러그인 행은 원두 아이콘(추출중=진한 브라운/그 외 연한 탠) + dotted leader.
- 삭제(🗑 uninstall)는 파일을 지우는 파괴적 동작 — **인라인 확인 단계**("삭제할까요? [삭제][취소]")를 거친 뒤 실행하고, 완료되면 행이 **재료 없음 + [담기]** 상태로 되돌아간다 (org 카탈로그에는 남아 있으므로 목록에서 사라지지 않음).
- **고정(pin) / 미고정 모드** (요구 4):
...
- **org 추가 UX (팝오버)**: 챗 전송 버튼 왼쪽의 ☕ 커피가루 아이콘("커피가루 담기") 클릭 → 위로 팝오버("원두(org)를 포터필터에 담아요" — URL 입력+추가) → 검증 중 스피너 → 성공 시 팝오버 닫힘 + 카드 추가 + 자동 스캔 / 실패 시 팝오버 안 인라인 사유("권한 없음", "다른 서버" 등). **바깥 클릭·Esc로 아이콘으로 접힘**. 미검증 세션에서도 사용 가능(§10.2 — 유일하게 열린 기능). org 카드 헤더에 새로고침(재스캔)·제거 버튼.
