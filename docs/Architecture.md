...
- **org 추가 UX (팝오버)**: 챗 전송 버튼 왼쪽의 🔗 아이콘 클릭 → 위로 팝오버(URL 입력+추가) → 검증 중 스피너 → 성공 시 팝오버 닫힘 + 그룹 추가 + 자동 스캔 / 실패 시 팝오버 안 인라인 사유("권한 없음", "다른 서버" 등). **바깥 클릭·Esc로 아이콘으로 접힘**. 미검증 세션에서도 사용 가능(§10.2 — 유일하게 열린 기능). org 헤더에 새로고침(재스캔)·제거 버튼.
