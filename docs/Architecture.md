...
flowchart TD
    ...
    F -- 예 --> H
    F -- 아니오 --> G["로그인 창: ID + PWD(PAT)<br>성공 시 credentials.json에 자동 저장"]
...
- 로그인 성공 후에만 org 등록·스캔·설치 기능이 열린다. **PAT는 필수**다(§10.2).
- **org는 권한이 확인된 것만 등록·표시된다** — 등록 시 1회 + 매 시작 시 재검증(§10.2).
...
| `services/catalog_service.py` | `pm list`: 등록 org 전체(또는 지정 org) 스캔 → 태그 필터 → catalog 저장. `--cached` 경로 | — |
...
| `api/lifecycle.py` | `POST /api/heartbeat`, `POST /api/shutdown` — 서버 수명 관리 (§12.5) | — |
...
pm update  [name]    → git pull → 재설치 트리거 (§6.3 캐시 특성 흡수)
```
...
- `plugins/{org}/{name}`에서 `git pull`만 해서는 반영되지 않는다 → `pm update` = `git pull` + 재설치로 흡수, `pm inspect`가 clone HEAD와 설치본의 차이를 표시.
...
| `pm org remove <org>` | 등록 해제 (설치된 플러그인은 유지, 경고 표시) | |
| `pm list` | 등록 org 전체 스캔(§10) → 태그 필터 → catalog 갱신 → **org별 그룹** 표 출력 | `--org <name>`, `--cached`, `--all`(태그 필터 해제), `--json` |
...
- 자동 로그인이라도 **검증 체인은 매번 수행**된다 — 권한을 잃으면 로그인 창으로 돌아간다.
...
- 빠른 경로: `"$PYTHON" -c "import flask, flask_sock, requests"` (requirements의 전 패키지) 성공 시 설치 전체 생략 (멱등·오프라인 친화, §9.0).
- user-site는 머신 전역 공유라 **버전 드리프트가 가능** → 격리 대신 체크리스트가 `importlib.metadata.version()`으로 요구 범위와 대조·감지한다.
- `env/requirements.txt`: `flask>=3.0`, `flask-sock`(터미널 WS), `requests>=2.31`, `claude-agent-sdk`(챗, §12.3), `pywinpty ; platform_system=="Windows"`(내장 터미널, §12.4).
...
| 5 | 패키지 버전 | import + `importlib.metadata` 범위 대조 (flask, flask-sock, requests, claude-agent-sdk, pywinpty[win]) | §9.2 설치 명령 |
...
- **host 결정 규칙**: 로그인 창에는 URL이 없으므로 —
  - `config.github_host`가 이미 있으면(과거에 org를 등록한 적 있음) 그 host로 즉시 검증.
  - **최초 실행(host 미정)이면 로그인 정보를 세션에 보관만 하고, 첫 org URL 추가 시점에 host 확정 → 그때 토큰·ID·멤버십을 한꺼번에 검증**한다. 실패하면 로그인 창으로 돌아간다.
...
| 터미널 WS 보호 | 내장 터미널은 임의 명령 실행 통로다 — localhost 바인딩에 더해, 로그인 세션에서 발급한 **1회용 세션 토큰을 WS 연결 시 검증** (다른 로컬 프로세스의 무단 연결 차단) |
...
│ 사이드바 │  메인 영역                                    │
│ (플러그인)│  ┌────────────────────────────────────────┐ │
│         │  │ 🤖 Claude 챗  (SSE 스트리밍, [새 대화])   │ │
│ 📌 고정  │  ├────────────────────────────────────────┤ │
│ 토글    │  │ 🖥️ 내장 터미널 (xterm.js — claude/pm     │ │
│         │  │    직접 실행, cwd=plugin_market)          │ │
│         │  └────────────────────────────────────────┘ │
└─────────┴──────────────────────────────────────────────┘
```

- **주 화면 = claude 챗 + 내장 터미널** (분할 기본, 탭 전환·비율 조절 옵션). 플러그인 관리는 왼쪽 사이드바로 이동.
- 단일 페이지(`web/index.html`) — 로그인 뷰 ↔ 메인 뷰를 JS로 전환. 프론트는 빌드 과정 없는 순수 HTML/CSS/JS.

### 12.2 사이드바 (플러그인 파트)

- 구성 (위→아래): **org URL 입력칸**(추가 버튼) → org별 플러그인 그룹(접기/펼치기) → 검색창 + 상태 필터 칩 → 플러그인 행(이름·설명·상태 배지·버튼) → Inspect 요약.
...
- **[새 대화] 버튼** 상시 표시. **플러그인 적용 시점 = 세션 시작 시** — Install/Enable/Disable은 새 세션부터 반영되므로, Install 직후 "새 대화부터 적용됩니다" 안내를 챗 상단에 표시.
- `pm …` 텍스트 입력은 챗이 가로채 services를 직접 호출(선택 편의 기능) — 결과를 챗 말풍선으로 표시.
...
· 페이지 JS가 2초 간격 POST /api/heartbeat  (탭별 세션 ID 포함 — 다중 탭 카운트)
· 탭 닫힘(pagehide) 시 navigator.sendBeacon("/api/shutdown", 세션 ID)
· 서버 watchdog: 활성 탭 수 == 0 이고 마지막 heartbeat로부터 10초 경과 → 자체 종료
  (유예 10초 = 새로고침·일시적 백그라운드 전환 오탐 방지)
· 종료 시 정리: pty 세션 종료 → SDK 챗 세션 정리 → 프로세스 exit
```

- 검토 결론: **가능** — heartbeat + sendBeacon + 유예의 조합이 브라우저 종류와 무관하게 동작하는 표준 패턴이다. sendBeacon은 즉시 종료 신호(빠른 경로), heartbeat 소실은 보장 경로(sendBeacon이 유실돼도 10초 내 종료).
...
- **자동 저장(기본 동작)**: 로그인 성공 시 `data/credentials.json`에 기록(§8.4). 다음 시작부터는 파일을 읽어 **로그인 창 없이 바로 메인 화면** — 단 검증 체인은 매번 수행되어 토큰 만료·권한 상실 시 로그인 창으로 폴백(입력값 프리필).
